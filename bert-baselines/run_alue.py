        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (
                ...
                if (args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0) \
                    or (args.save_epochs and (step + 1) == len(epoch_iterator)):
                    # Save model checkpoint
                    if (step + 1) == len(epoch_iterator):
                        checkpoint_name = "checkpoint-epoch-{}".format(epoch_index + 1)
                    else:
                        checkpoint_name = "checkpoint-step-{}".format(global_step)
...
def evaluate(args, model, tokenizer, prefix="", test=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if submit:
            ...
            processor.create_submission(os.path.join(eval_output_dir, prefix), preds)

        # Special case, create submission for xnli dev set (not just test set)
        if args.task_name == 'xnli' and not submit:
            processor = processors[eval_task]()
            processor.create_submission(os.path.join(eval_output_dir, prefix), preds, test=False)
...
def main():
    ...
    if args.output_mode == 'multilabel':
        ...
        pretrained_model_prediction_type = AutoModelForSequenceClassification

    model = pretrained_model_prediction_type.from_pretrained(
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = pretrained_model_prediction_type.from_pretrained(args.output_dir)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = pretrained_model_prediction_type.from_pretrained(checkpoint)
            model.to(args.device)
            if args.task_name != "mq2q":
            ...
            if args.task_name in ["mq2q", "svreg", "sec", "oold", "ohsd", "xnli"]:
                evaluate(args, model, tokenizer, prefix=prefix, test=True)
